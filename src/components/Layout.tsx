...
const Layout: React.FC = () => {
  ...
  return (
    <div className="relative flex h-screen font-sans bg-[#0B0F19] selection:bg-blue-500/30 overflow-hidden">
      <AuroraModalBackground />
      {/* Sidebar - Multiplier AI Style */}
      <aside
      onMouseEnter={() => setIsSidebarHovered(true)}
      onMouseLeave={() => setIsSidebarHovered(false)}
      className={`${!isEffectivelyExpanded ? 'w-20' : 'w-72'} border-r border-white/10 bg-white/5 backdrop-blur-xl hidden md:flex flex-col z-10 transition-all duration-300 shrink-0`}>

        {/* Header da Sidebar */}
        <div className={`h-12 flex items-center px-4 border-b border-white/10 ${!isEffectivelyExpanded ? 'justify-center' : 'justify-between'}`}>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-blue-800 hover:scrollbar-thumb-slate-400 dark:hover:scrollbar-thumb-blue-700">
          ...
          <div className="p-3 space-y-1">
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 h-8 px-3 rounded-md transition-all duration-200 text-sm font-medium ${isActive('/files') ? 'bg-white/10 text-white border-r-2 border-white/50' : 'text-gray-300 hover:bg-white/10 hover:text-white'}`}
                title="Arquivos"
              >
                <FileText size={20} className={isActive('/files') ? 'text-white' : ''} />
                {isEffectivelyExpanded && <span>Arquivos</span>}
              </button>
              <button
                onClick={() => navigate('/projects')}
                className={`w-full flex items-center gap-3 h-8 px-3 rounded-md transition-all duration-200 text-sm font-medium ${isActive('/projects') ? 'bg-white/10 text-white border-r-2 border-white/50' : 'text-gray-300 hover:bg-white/10 hover:text-white'}`}
                title="Projetos"
              >
                <Folder size={20} className={isActive('/projects') ? 'text-white' : ''} />
                        ...
                        {conversations.map(chat => (
                            <div key={chat.id} className="relative">
                                <div
                                    ...
                                    className={`group relative flex items-center justify-between h-8 px-3 rounded-md cursor-pointer transition-colors ${activeConversationId === chat.id ? 'bg-white/10 text-white' : 'hover:bg-white/10 text-gray-300 hover:text-white'}`}
                                >
                                ...
                                {openChatMenuId === chat.id && createPortal(
                                    <div
                                        ...
                                        className="w-60 dark: backdrop-blur-xl border border-white/10 rounded-xl shadow-xl py-2"
                                    >
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
        ...
        <div className="p-3 border-t border-white/10">
          <div className="relative" ref={userMenuRef}>
             ...
             {/* User Menu Dropdown */}
             {showUserMenu && (
                <div className="absolute bottom-full left-0 w-full mb-2 dark: backdrop-blur-xl border border-white/10 rounded-lg shadow-xl py-1 overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-200 z-50">
                    <button
